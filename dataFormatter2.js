...
var option2 = function() {
    ...
    getFestival(twentyOnePilotsQueryInfo);
};
...
var processFestivalData = function(data, start, end){
    var finalArray = [];
...
var processConcertData = function(data, start, end){
    var finalArray = [];
    var concertDate  = start;  //Month is 0-11 in JavaScript
    var ticketsOnSale  = end; //Month is 0-11 in JavaScript
    ...
    for(var i =0; i<data.length; i++){
        ...
        for(var j = 0; j<obs.section_stats.length; j++){
            if(obs.section_stats[j].sectionName.indexOf("Floor General Admission")>=0){
                gaIndex = j;
                break;
            }
        }
        ...
        if(gaIndex == -1) {
            ...
            observation.GAmin = obs.section_stats[gaIndex].minTicketPrice;
            observation.GAmax = obs.section_stats[gaIndex].maxTicketPrice;
            observation.GAavg = obs.section_stats[gaIndex].averageTicketPrice;
            observation.GAamount = obs.section_stats[gaIndex].totalTickets;
            observation.GAnumListings = obs.section_stats[gaIndex].totalListings;
        }
        ...
        var timeTillFest = concertDate - obs.dateQueried.dateObject;
        observation.daysTillFest = timeTillFest/one_day;
        observation.hoursTillFest = Math.round(timeTillFest/one_hour);
...
var processData = function(){
    ...
    for (var key in festivals) {
        ...
        for (var day in festival)  {
            ...
            switch(key){
                case 'lollapalooza':
                    data = processFestivalData(festival[day].theData, new Date(2016, 6, 28), new Date(2016, 2, 22));
                    break;
                case 'governorsBall':
                    data = processFestivalData(festival[day].theData, new Date(2016, 5, 3), new Date(2016, 1, 20));
                    break;
                case 'bonnaroo':
                    data = processFestivalData(festival[day].theData, new Date(2016, 5, 9), new Date(2015, 10, 27));
                    break;
                case 'springAwakening':
                    data = processFestivalData(festival[day].theData, new Date(2016, 5, 13), new Date(2016, 1, 1));
                    break;
                case 'outsidelands':
                    data = processFestivalData(festival[day].theData, new Date(2016, 7, 5), new Date(2016, 2, 28));
                    break;
                case 'twentyOnePilots':
                    data = processConcertData(festival[day].theData, new Date(2016, 7, 5), new Date(2016, 2, 28));
                    break;
                default:
